...
const ProductButton = ({ item }) => {
  ...
  const { setCurrentUpdatedProduct, setComponentLevelLoader, componentLevelLoader, user } = useContext(GlobalContext);
  ...
  return isAdminView ? (
    ...
    <>
      <button onClick={() => handleAddToCart(item)} className='mt-1.5 flex w-full justify-center items-center bg-black px-5 py-3 font-medium uppercase tracking-wide text-white'>Add to Cart</button>
    </>
  )
