import { ConnectDB } from "@/database/Connection";
import AuthUser from "@/middleware/AuthUser";
import Order from "@/model/Order";
import { NextResponse } from "next/server";


export const dynamic = "force-dynamic";

export async function GET (req) {
    try {
        await ConnectDB();

        const isAuthUser = await AuthUser(req);
        if(isAuthUser) {
            const {searchParams} = new URL(req.url);
            const id = searchParams.get('id');
            if(!id) {
                return NextResponse.json({message: "Product ID required", success: false});
            }

            const extractOrderDetails = await Order.findById(id).populate('orderItems.product');
            if(extractOrderDetails){
                return NextResponse.json({data: extractOrderDetails, success: true});
            } else {
                return NextResponse.json({message: "Failed to get product details", success: false});
            }
        } else {
            return NextResponse.json({message: "You are not Authenticated", success: false});
        }
        
    } catch (error) {
        return NextResponse.json({message: "Something went wrong while getting order details", success: false});
    }
}